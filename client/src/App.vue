<template>
  <div id="app">
    <nav class="cont">
      <h1><a class="logo-link" href="/">PUB QUIZ BUDDY</a></h1>
    ...
    <pub-quiz-map
      ...
      :quizzes="quizzes"
      :selectedDayQuizzes="selectedDayQuizzes"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    fetchQuizzes() {
      PubQuizzesService.getQuizzes().then(
        fetchedQuizzes => (this.quizzes = fetchedQuizzes)
      );
    },
    displayForm(event) {
      this.showModal = true;
  ...
  mounted() {
    ...
    eventBus.$on("selected-day", stringOfDayClicked => {
      ...
      PubQuizzesService.getQuizzesByDay(this.selectedDay).then(
        fetchedQuizzes => (this.selectedDayQuizzes = fetchedQuizzes)
      );
    });
    ...
    eventBus.$on("pub-quiz-added", newQuiz => {
      ...
      PubQuizzesService.getQuizzesByDay(newQuiz.day).then(
        fetchedQuizzes => (this.selectedDayQuizzes = fetchedQuizzes)
      );
    });
  },
